...
class TestCogVideoX:
    def test_forward_shape(self):
        ...
        model = CogVideoX(
            ...
            num_experts=2, patch_size=(1, 2, 2), num_frames=4,
            image_size=8, context_dim=32,
        )
...
class TestVideoMAE:
    def test_pretrain_forward(self):
        ...
        model = VideoMAE(
            in_channels=3, embed_dim=64, encoder_depth=2, decoder_dim=32,
            decoder_depth=1, num_heads=4, decoder_heads=2,
            patch_size=8, tubelet_length=2, num_frames=4,
            image_size=32, mask_ratio=0.75,
        )
    ...
    def test_finetune_forward(self):
        ...
        model = VideoMAE(
            in_channels=3, num_classes=10, embed_dim=64,
            encoder_depth=2, decoder_dim=32, decoder_depth=1,
            num_heads=4, decoder_heads=2, patch_size=8,
            tubelet_length=2, num_frames=4, image_size=32,
        )
        x = torch.randn(2, 3, 4, 32, 32)
...
class TestSlowFast:
    def test_forward(self):
        ...
        model = SlowFast(
            in_channels=3, num_classes=10, slow_channels=16,
            fast_channels=4, alpha=4, layers=(1, 1, 1, 1),
        )
